...
namespace VCX::Labs::FluidSimulation {
    ...
    class CaseVariation : public Common::ICase {
    public:
        CaseVariation(std::initializer_list<Assets::ExampleScene> && scenes);

        virtual std::string_view const GetName() override { return "Variational Simulation"; }
    ...
    private:
        ...
        float                               _r;
        int                                 numofSpheres;
        Final::VariationalSimulator*        _sim;
        Final::VariationalSimulator         _simulation;
